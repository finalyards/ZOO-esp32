...
#
if grep -q '"esp32c3"' drv8871/Cargo.toml; then
  MCU=esp32c3
elif grep -q '"esp32c6"' drv8871/Cargo.toml; then
  MCU=esp32c6
else
  (echo >&2 "Error parsing 'drv8871/Cargo.toml'; please set up manually!"; false)
fi
...
cp drv8871/Cargo.toml tmp-2
cat tmp-2 | sed -E "s/(\")esp32c[36](\")/\1${MCU}\2/g" \
  > drv8871/Cargo.toml

rm tmp-[12]

echo "Files '.cargo/config.toml' and 'drv8871/Cargo.toml' now using:"
echo ""
echo "   MCU:    ${MCU}"
echo "   TARGET: ${TARGET}"
echo ""
